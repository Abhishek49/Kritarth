	...
	( function ( ) {
		var pageSwitchHandler = {
			...
			goForwardToPage : function ( pageID , durationTime ) {
				animationEffects . fadeOutElement ( document . getElementById ( "main-div" ) , { complete : function ( ) {
					variableListeners . setListeners ( false ) ;
					if ( window . innerHeight > window . innerWidth ) durationTime = durationTime || 100 ;
					else durationTime = durationTime || 100 ;
					...
					loaderHandler . hideLoader ( null , { complete : function ( ) { animationEffects . scrollBodyIntoView ( { scrollTop : document . getElementById ( pageID ) . offsetTop , scrollLeft : document . getElementById ( pageID ) . offsetLeft } , { duration : durationTime , complete : function ( ) { animationEffects . fadeInElement ( document . getElementById ( "main-div" ) ) ; variableListeners . setListeners ( true ) ; } } ) ; } } ) ;
				} } ) ;
			} ,
			goToSpecificPage : function ( pageDetails , durationTime ) {
				animationEffects . fadeOutElement ( document . getElementById ( "main-div" ) , { complete : function ( ) {
					...
					loaderHandler . hideLoader ( null , { complete : function ( ) { if ( typeof pageDetails . page === 'string' ) animationEffects . scrollBodyIntoView ( { scrollTop : document . getElementById ( pageDetails . page ) . offsetTop , scrollLeft : document . getElementById ( pageDetails . page ) . offsetLeft } , { duration : durationTime , complete : function ( ) { animationEffects . fadeInElement ( document . getElementById ( "main-div" ) ) ; variableListeners . setListeners ( true ) ; } } ) ; } } ) ;
				} } ) ;
			} ,
		...
		customEventsHandler . addListenerEvent ( window , "touchmove" , function ( ) {
			pageSwitchHandler . goToSpecificPage ( { page : pageSwitchHandler . currentPageID , title : document . title } ) ;
		} ) ;
	} ) ( ) ;
		...
		var pageAJAXLoaderHandler = {
			...
			goForwardToPage : function ( pageID , pageAJAXURL , durationTime ) {
				animationEffects . fadeOutElement ( document . getElementById ( "main-div" ) , { complete : function ( ) {
					...
					pageAJAXLoaderHandler . AJAXRequestPage ( pageAJAXURL , function ( responseText ) {
						if ( pageAJAXLoaderHandler . AJAXRequestSuccess ( responseText , { pageID : pageID } ) ) {
							pageAJAXLoaderHandler . currentPageID = pageID ;
							pageSwitchHandler . currentPageID = pageID ;
						...
						var errorMessage = pageAJAXLoaderHandler . AJAXRequestFailure ( responseStatus , responseText ) ;
						window . setTimeout ( function ( ) {
			...
			goToSpecificPage : function ( pageDetails , durationTime ) {
				animationEffects . fadeOutElement ( document . getElementById ( "main-div" ) , { complete : function ( ) {
					...
					if ( typeof pageDetails . page === 'string' ) {
						pageAJAXLoaderHandler . AJAXRequestPage ( pageDetails . pageURL , function ( responseText ) {
							if ( pageAJAXLoaderHandler . AJAXRequestSuccess ( responseText , { pageID : pageDetails . page } ) ) {
								pageAJAXLoaderHandler . currentPageID = pageDetails . page ;
								pageSwitchHandler . currentPageID = pageDetails . page ;
								...
								window . setTimeout ( function ( ) {
									loaderHandler . hideLoader ( null , { complete : function ( ) { animationEffects . scrollBodyIntoView ( { scrollTop : document . getElementById ( pageDetails . page ) . offsetTop , scrollLeft : document . getElementById ( pageDetails . page ) . offsetLeft } , { duration : durationTime , complete : function ( ) { animationEffects . fadeInElement ( document . getElementById ( "main-div" ) ) ; variableListeners . setListeners ( true ) ; } } ) ; } } ) ;
								} , 3000 ) ;
							}
						} , null , function ( responseStatus , responseText ) {
							var errorMessage = pageAJAXLoaderHandler . AJAXRequestFailure ( responseStatus , responseText ) ;
	...
	( function ( ) {
		formAJAX = {
			...
			submitFormSuccess : function  ( formObj , responseText ) {
				var formAJAXResults = null ;
				try {
					formAJAXResults = JSON . parse ( responseText ) ;
					var formResultDivID = ( formObj . id ) + '-message-result' ;
					var formLoadDivID = ( formObj . id ) + '-loading-image' ;
					...
					formObj . insertAdjacentHTML ( 'beforebegin' , '<div id="' + formResultDivID + '" class="block-section full-width no-overflow border-radius-xsmall" style="color: #D8000C;background-color: #FFBABA;">' + responseText + '</div>' ) ;
				}
			} ,
